...
const TechTestModal: React.FC<TechTestModalProps> = ({ open, onClose, project  }) => {
    ...
    const sendTechnicalTest = async (technicalTestPayload: any) => {
      try {
        const response = await fetch(`${API_URL}/positions/${technicalTestPayload.position}/tests`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${session?.user?.token}`,
    ...
    const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      try {
        await validationSchema.validate(technicalTestPayload, { abortEarly: false });
        ...
        if (err instanceof yup.ValidationError) {
          const newErrors = err.inner.reduce((acc, currentError) => {
            ...
            if (errorPath === "position" || errorPath === "candidate" || errorPath === "observations") {
            ...
            <Box sx={{ display: 'flex', width: '100%' }}>
                ...
                <Box sx={{ flexGrow: 1, mr: 2 }}>
                    ...
                    <FormControl variant="standard" sx={{ m: 1, width: '90%' }} size="small">
                        ...
                        >
                            {project && project.positions.map((position) => (
